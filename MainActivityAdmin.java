...
public class MainActivityAdmin extends AppCompatActivity {
        ...
        fireBaseArchive.getAllRequests(new OnCompleteListener<QuerySnapshot>() {
            ...
            public void onComplete(@NonNull Task<QuerySnapshot> task) {
                if (task.isSuccessful()) {
                    ...
                    createListApprovate(requestBeans);
                    createListRifiutata(requestBeans);
                    createListInviata(requestBeans);
    ...
    private void createListApprovate(ArrayList<RequestBean> requestBeans) {
        for (RequestBean r : requestBeans) {
            if (r.isStato().equalsIgnoreCase("approvata"))
                approvata.add(r);
        }
    }


        private void createListRifiutata(ArrayList<RequestBean> requestBeans) {
            for (RequestBean r : requestBeans) {
                if (r.isStato().equalsIgnoreCase("rifiutato"))
                    rifiutata.add(r);

                /*Intent i=new Intent(getApplicationContext(),RequestAdapterAdmin.class);
                i.putExtra("rifiuta",rifiutata);*/
    }
    }


            private void createListInviata(ArrayList<RequestBean> requestBeans) {
                for (RequestBean r : requestBeans) {
                    if (r.isStato().equalsIgnoreCase("inviato"))
                        inviata.add(r);
                }
    ...
    public void excelApproved() {
      /* RetrofitSingleton.getInstance().performCreateApprovedExcel(new Callback<Void>() {
        ...
        Intent i = new Intent(getApplicationContext(), ExcelApproved.class);
        i.putExtra("approvata",approvata);
        startActivity(i);
    ...
    public void excelRefused() {
      /*  RetrofitSingleton.getInstance().performCreateRefusedExcel(new Callback<Void>() {
        ...
        Intent intent = new Intent(getApplicationContext(), ExcelRefused.class);
        intent.putExtra("rifiutata",rifiutata);
        startActivity(intent);
